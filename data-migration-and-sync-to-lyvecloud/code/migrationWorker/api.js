...
function apiGetNewTaskToRun() {
    return axios.get(`http://${process.env.BACKEND_IP}:10086/common/new-migration-task-to-run`);
}

function apiUpdateMigrationTaskProgress(id, data) {
    return axios.put(`http://${process.env.BACKEND_IP}:10086/migration-tasks/${id}`, data);
}

function apiUpdateTaskPartitionProgress(id, partitionIndex, data) {
    return axios.put(`http://${process.env.BACKEND_IP}:10086/migration-tasks/${id}/partitions/${partitionIndex}`, data);
}

function apiPostTaskLog(id, log) {
    return axios.post(`http://${process.env.BACKEND_IP}:10086/migration-tasks/${id}/logs`, log);
}
