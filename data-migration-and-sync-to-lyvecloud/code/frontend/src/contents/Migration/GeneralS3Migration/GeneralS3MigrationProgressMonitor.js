...
import MigrationTaskDetailDrawer from "./MigrationTaskDetailDrawer";
import migrationTaskTableColumns from "./MigrationTaskTableColumns";
...
function GeneralS3MigrationProgressMonitor({tasks}) {
    ...
    return <div>
        ...
        <Table dataSource={filterTasks(filters, tasks)} columns={migrationTaskTableColumns(setTaskToView)}
               size={"small"}
               pagination={{position: ["bottomLeft"], defaultPageSize: 100}}/>
        <MigrationTaskDetailDrawer setTaskToView={setTaskToView} taskToView={taskToView}/>
    </div>;
}
