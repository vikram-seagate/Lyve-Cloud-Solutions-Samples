...
import GoogleBucketSynchronizationForm from "./GoogleBucketSynchronizationForm";
import GoogleSynchronizationProgressMonitor from "./GoogleSynchronizationProgressMonitor";
...
function GoogleSynchronization() {
    ...
    const loadTasks = () => {
        ...
        apiFetchSynchronizationJobs("fromGoogle")
            .then(res => {
                console.log(res.data);
    ...
    return <div style={styles.pageContainer}>
        <div style={{display: "flex", justifyContent: "space-between"}}>
            <h1>Google Cloud Storage → Lyve Bucket Synchronization Dashboard
            </h1>
            <div>
        ...
        <Tabs defaultActiveKey="monitor">
            <TabPane tab="Progress Monitor" key="monitor">
                <GoogleSynchronizationProgressMonitor syncTasks={syncTasks}
                                                      setSyncTasks={setSyncTasks}
                                                      syncJobs={syncJobs}
                                                      setSyncJobs={setSyncJobs}/>
            </TabPane>
            <TabPane tab="Synchronization Configuration">
                <GoogleBucketSynchronizationForm/>
            </TabPane>
        </Tabs>
    </div>;
}

export default GoogleSynchronization;