...
import AlibabaBucketSynchronizationForm from "./AlibabaBucketSynchronizationForm";
import AlibabaSynchronizationProgressMonitor from "./AlibabaSynchronizationProgressMonitor";
...
function AlibabaSynchronization() {
    ...
    const loadTasks = () => {
        ...
        apiFetchSynchronizationJobs("fromAlibaba")
            .then(res => {
                console.log(res.data);
    ...
    return <div style={styles.pageContainer}>
        <div style={{display: "flex", justifyContent: "space-between"}}>
            <h1>Alibaba OSS Bucket → Lyve Bucket Synchronization Dashboard
            </h1>
            <div>
        ...
        <Tabs defaultActiveKey="monitor">
            <TabPane tab="Progress Monitor" key="monitor">
                <AlibabaSynchronizationProgressMonitor syncTasks={syncTasks}
                                                       setSyncTasks={setSyncTasks}
                                                       syncJobs={syncJobs}
                                                       setSyncJobs={setSyncJobs}/>
            </TabPane>
            <TabPane tab="Synchronization Configuration">
                <AlibabaBucketSynchronizationForm/>
            </TabPane>
        </Tabs>
    </div>;
}

export default AlibabaSynchronization;